...
class RadioProvider with ChangeNotifier, WidgetsBindingObserver {
  ...
  static const String _keySleepTimerDuration = 'sleep_timer_duration_v2';
  ...
  void setSleepTimerEnabled(bool enabled) {
    ...
    if (!enabled) {
      _sleepTimer?.cancel();
      _sleepTimer = null;
      _audioHandler.customAction('setVolume', {'volume': 1.0});
    } else {
      // Use the set duration
      _remainingSleepTime = _sleepTimerDuration;
      _targetSleepTime = DateTime.now().add(_sleepTimerDuration);
      _startSleepTimer();
    }
    notifyListeners();
    _saveTimerState();
  ...
  void setSleepTimerDuration(Duration duration) {
    ...
    _targetSleepTime = DateTime.now().add(duration);
  ...
  void _startSleepTimer() {
    ...
    _sleepTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (!_sleepTimerEnabled || _targetSleepTime == null) {
        timer.cancel();
        // Ensure volume is reset if timer is canceled manually
        _audioHandler.customAction('setVolume', {'volume': 1.0});
        return;
      }

      final now = DateTime.now();
      final remaining = _targetSleepTime!.difference(now);

      if (now.isAfter(_targetSleepTime!)) {
        timer.cancel();
        _sleepTimerEnabled = false;
        notifyListeners();
        _stopAndExit();
        ...
        if (remaining.inSeconds <= 10) {
          final double volume = remaining.inSeconds / 10.0;
          _audioHandler.customAction('setVolume', {'volume': volume.clamp(0.0, 1.0)});
        }

        if (_remainingSleepTime!.inSeconds % 60 == 0 || remaining.inSeconds <= 10) {
          notifyListeners();
        }
      }
    });
  ...
  void _stopAndExit() async {
    LogService().log("Sleep Timer Expired: Stopping playback and exiting app.");
    try {
  ...
  Future<void> _saveTimerState() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt(
      _keySleepTimerDuration,
      _sleepTimerDuration.inSeconds,
    );
  }

  Future<void> _loadTimerState() async {
    final prefs = await SharedPreferences.getInstance();
    final sleepSecs = prefs.getInt(_keySleepTimerDuration);
    if (sleepSecs != null) {
      _sleepTimerDuration = Duration(seconds: sleepSecs);
      _remainingSleepTime = _sleepTimerDuration;
    }
    notifyListeners();
  }
  ...
  RadioProvider(
    ...
    _onEntitlementChanged(); // Initial sync
  ...
  Future<void> restoreBackup({bool isFullReplace = false}) async {
    ...
    try {
      ...
      if (jsonStr == null) {
        LogService().log("[RadioProvider] No backup found on Google Drive. Initiating first background backup.");
        // Se non esiste un backup, marchiamo il restore come "fatto" (anche se vuoto)
        ...
        performBackup(isAuto: true).catchError((e) {
          LogService().log("[RadioProvider] First background backup failed: $e");
        });
  ...
  void _onEntitlementChanged() {
    ...
    // SAFETY: If the sleep_timer feature is disabled, force stop any active timer
    if (!_entitlementService.isFeatureEnabled('timer_alarm')) {
      if (_sleepTimerEnabled) {
        LogService().log(
          "RadioProvider: Feature 'sleep_timer' disabled via Remote Config. Forcing stop of active timer.",
        );
        clearSleepTimer();
      }
    }
